...
### Fixed — PWA QR pairing regression: lookup-null falls through to home relay
- Reverts the narrow "lookup-null without cache confirmation → persist to `pending_sends`" branch introduced in the prior "durable cross-relay sends" fix. That branch broke QR pairing: first-contact bootstrap envelopes legitimately have no presence-cache entry and often see a null `peer_lookup` result if the provider's presence hasn't propagated yet, so the bootstrap was silently queued instead of sent. The scanner's UI reported "connected" (the scan itself succeeded) while the provider never received the envelope.
- `sendRawEnvelope` now treats `homeRelay === null` the same as `homeRelay === connMgr.relayUrl` — route through our own home relay, which delivers directly on common-case single-relay deployments and falls back to "peer not connected" if it genuinely can't deliver, letting the caller retry. Cross-relay failure queueing (openOutbound / acquireToken / sendToRelayUrl) remains intact, `flushMessageQueue` still routes through the full `sendRawEnvelope` path, and `MAX_QUEUE_AGE_MS = 24h` is unchanged.
- One unit test replaced (not added): `lookup-null with unknown home-relay cache persists instead of misrouting` → `lookup-null falls through to home-relay send (QR-pairing first-contact path)`. New test asserts (a) lookup-null invokes the home-relay send with the correct peer/envelope, (b) home-relay send failures still persist to `pending_sends` so the durability net for genuine home-relay failures is intact. The other three durability tests (openOutbound failure → persist, flush re-runs end-to-end, 24h horizon) are unchanged. **PWA tests: 108/108 green.**

### Fixed — PWA durable cross-relay sends + 24h retry horizon
- Cross-relay send failures now persist to `pending_sends` via `queueMessage()` instead of toast-and-drop, so `flushMessageQueue` can retry them with a refreshed peer_lookup. Peer-lookup-null no longer silently routes to sender's own home relay when that relay isn't known (via the presence cache) to be peer's home. `MAX_QUEUE_AGE_MS` raised from 1h to 24h to match the relay's store-and-forward TTL. `flushMessageQueue` now routes queued entries through the full `sendRawEnvelope` path (WebRTC → onion → home-relay → cross-relay) instead of a home-relay-only retry, so a now-reachable peer-lookup gets a second chance. `sendRawEnvelope` gained an optional `{ noPersist }` flag so the flush path doesn't double-queue its own retries. 4 new unit tests cover openOutbound failure → persist, lookup-null with empty cache → persist (no misroute), flush re-runs sendRawEnvelope end-to-end, and the 24h-vs-1h horizon.
...
- Relay-server: the `IssueLimiter` type is kept as a unit-typed placeholder so a future re-enable doesn't touch the handler signature. The 429 branch is gone.
- PWA: `DEFAULT_BATCH_SIZE` goes from 256 back up to 1024 now that the per-epoch cap is no longer a constraint — fewer round-trips without any abuse-control downside. `LOW_WATER` rises to 128 to match.
- Conformance: MUST-063 test retagged as the MAY-005 shape-conformance test — a relay that does re-enable the limiter has a pin to target. Workspace: 1253/1253 green.

### Changed — Spec/code drift cleanup: token-issuance accounting + federation admission
- **PNP-001 v0.7:** Added `PNP-001-MUST-063` to §10.2 making the `budget_per_epoch` cap explicitly cumulative per (identity, epoch). Prior text referenced the 8192 cap only in the request-body schema comment, leaving room to interpret the limit as "one batch per epoch" — which had been the implementation until the earlier `fix(tokens)` commit loosened it. The new clause locks the semantic: multiple issuance requests MUST be accepted while the running total stays under the cap; overflow returns 429 and MUST NOT advance the counter. Clause-pinned conformance test added in `pnp_001_outer_frame.rs`. PNP-001 MUST count: 63.
- **Federation admission cleanup:** the `/federation/v1` handler previously admitted inbound peers based on a claimed `X-Parolnet-Peer-Id` HTTP header — no cryptographic proof, and a spoofed value could dedup-kick a legitimate peer (claim their id, trip MUST-083 close 4000). Header admission removed. The handler now accepts the WSS upgrade, runs the frame-decode loop with no `FederationManager` admission, and closes on codec errors per MUST-079/080. Admission (MUST-083 dedup, reputation accounting) now lives only in `FederationLink::admit_inbound` unit tests; wire-level admission lands alongside the PNP-002 handshake (MUST-018), which was always the spec-mandated path. Dropped the `X-Parolnet-Peer-Id` header forwarding from `server/nginx.conf` and the unused `FederationManager` instantiation from relay-server main.
- Workspace: 1253/1253 green (one new conformance test for MUST-063).
